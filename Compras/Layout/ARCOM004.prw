...
±±ºPrograma  ³ARCOM004        ºAutor  ³Microsiga     ºFecha ³  01/14/05   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ IMPRESIÓN DE PEDIDO DE COMPRAS                             º±±
±±º          ³                                                            º±±
...
User Function ARCOM004(_cNumPedCom)

Local cDesc1  		:= "Este programa tiene como objetivo imprimir "
Local cDesc2  		:= "la orden de compra. "
Local NomeProg		:= "ARCOM004"
Local nLastKey    	:= 0
Local lAbortPrint	:= .F.
Private cPerg 		:= padr("ARCOM004",10)
...
oPrn:SetPortrait()
If lAbortPrint
	@nLin,00 PSAY "*** CANCELADO POR EL OPERADOR ***"
	Return
Endif
...
While !Eof()
	If nLin > 2400 .Or. IS2->C7_NUM <> cNumPedido
		If !lPrimera
...
//	_cDescri := Posicione("SB1",1,Xfilial("SB1")+IS2->C7_PRODUTO,"B1_DESC")
	...
	oPrn:Say( nLin, 1950, TRANSFORM((IS2->C7_TOTAL *(1-(IS2->C7_DESC1/100))*(1-(IS2->C7_DESC2/100))),"@E 99,999,999.99"),oFont10,070 )
	...
	ENDiF
		If IS2->C7_ALQIMP6 <> 0
		nTotIIBA		:=	nTotIIBA + (IS2->C7_TOTAL *(1-(IS2->C7_DESC1/100))*(1-(IS2->C7_DESC2/100))*(IS2->C7_ALQIMP6/100))
	ELSEIF  IS2->C7_ALQIMP5 <> 0
		nTotIIBB		:=	nTotIIBB + (IS2->C7_TOTAL *(1-(IS2->C7_DESC1/100))*(1-(IS2->C7_DESC2/100))*(IS2->C7_ALQIMP5/100))
	ENDiF
	nTotTotal   :=   nTotsIva + nTotIva +nTotIIBA + nTotIIBB
	nDesctot	:= nTotTotal * (IS2->C7_DESC3 /100)
	_nRegObs	:= mlcount(Alltrim(_cObserv),140)
	For nx	:= 1 to _nRegObs
		If nLin > 1600
			oPrn:Say( nLin+300, 2000, " *** CONTINUA EN LA SIGUIENTE PAGINA *** " ,oFont09,070 )
			oPrn:EndPage()
			_cmoeda		:= STR(IS2->C7_MOEDA)
			Crea_Pagina()
			nLin := 600 //Detalle
			If Nx == 1  .and. !empty(_cObserv)
				oPrn:Say( nLin, 0010, IF(MV_PAR03==1,"Observaciones: ",''),oFont08,100 )
				oPrn:Say( nLin, 0200, IF(MV_PAR03==1,memoline(_cObserv,140,nx),''),oFont10,100 )
				nLin += 50
			else
				oPrn:Say( nLin, 0010, IF(MV_PAR03==1,memoline(_cObserv,140,nx),''),oFont10,100 )
				nLin += 50
			endIf
		Else
			If Nx == 1  .and. !empty(_cObserv)
...
oPrn:Box( nLin, 1625,nLin+50* _nRenglones,1860 )
...
oPrn:Box( nLin, 1860,nLin+50* _nRenglones,2400 )
...
oPrn:Say( nLin, 0160, GETMV("MV_SIMB"+alltrim(_cmoeda)) + " " +GETMV("MV_MOEDAP"+Alltrim(_cmoeda)) ,oFont10,100 )
oPrn:Say( nLin, 1700, " Subtotal ",oFont10,100 )
oPrn:Box( nLin, 1700,nLin+55,2400 )//SubTotal
oPrn:Say( nLin, 2150, TRANSFORM(nTotsIva,"@E 99,999,999.99"),oFont10,100 )
oPrn:Say( nLin, 2010, GETMV("MV_SIMB"+alltrim(_cmoeda)),oFont10,100 )

nLin:=nLin + 55
oPrn:Say( nLin, 1700, " Iva ",oFont10,100 )
oPrn:Box( nLin, 1700,nLin+55,2400 )//Iva
oPrn:Say( nLin, 2010, GETMV("MV_SIMB"+alltrim(_cmoeda)),oFont10,100 )
oPrn:Say( nLin, 2150, TRANSFORM(nTotIva,"@E 99,999,999.99"),oFont10,100 )


nLin:=nLin + 55
oPrn:Say( nLin, 1700, " Ing.Brutos CF: ",oFont10,100 )
oPrn:Box( nLin, 1700,nLin+55,2400 )// Total
oPrn:Say( nLin, 2010, GETMV("MV_SIMB"+alltrim(_cmoeda)),oFont10,100 )
oPrn:Say( nLin, 2150, TRANSFORM(nTotIIBB,"@E 99,999,999.99"),oFont10,100 )
nLin:=nLin + 55
oPrn:Say( nLin, 1700, " Ing.Brutos BA: ",oFont10,100 )
oPrn:Box( nLin, 1700,nLin+55,2400 )// Total
oPrn:Say( nLin, 2010, GETMV("MV_SIMB"+alltrim(_cmoeda)),oFont10,100 )
oPrn:Say( nLin, 2150, TRANSFORM(nTotIIBA,"@E 99,999,999.99"),oFont10,100 )
nLin:=nLin + 55
oPrn:Say( nLin, 1700, " Total ",oFont10,100 )    
oPrn:Say( nLin, 2010, GETMV("MV_SIMB"+alltrim(_cmoeda)),oFont10,100 )
oPrn:Say( nLin, 2150, TRANSFORM(nTotTotal - nDesctot ,"@E 99,999,999.99"),oFont10,100 )
oPrn:Box( nLin, 1700,nLin+55,2400 )// Total
...
nLin	:= 2480
oPrn:Say( nLin, 0005, IF(MV_PAR03==1," Observaciones: ",'') ,oFont10,100 )
nLin := 2730    //Leyenda               
oPrn:Say( nLin, 0010, IF(MV_PAR03==1,_cXobserv,"" ),oFont10,100 )
oPrn:Say( nLin, 1900, Replicate("_",25) ,oFont10,100 )
oPrn:Say( nLin+50,2000, "Firma y Sello" ,oFont10,100 )
IF MV_PAR03==1
   oPrn:Box( nLin, 0005 ,nLin+200,1700 )
ENDIF
nLin += 250    //Leyenda
...
oPrn:Say( nLin, 0330, 'SUCESORES DE DOMINGO RESTA Y CIA S.A.', oFont09b, 100 )

//oPrn:Say( nLin, 2050, "Tel.: ( "+substr(SM0->M0_TEL,1,3) + ")" + Substr(SM0->M0_TEL,4,10) ,oFont08,100 )
oPrn:Say( nLin, 1850, iif(IS2->C7_CONAPRO=='L',"ORDEN DE COMPRA","PEDIDO PENDIENTE AUTORIZACION"),oFont10,100)
nLin:=110 

oPrn:Say( nLin , 0330, "DOM.: " + AllTrim( SM0->M0_ENDENT ) , oFont08,100)
//oPrn:Say( nLin , 0850, "DOM. COMERCIAL: " + AllTrim( SM0->M0_ENDCOB ), oFont10, 100)
oPrn:Say( nLin, 0850, "Tel./Fax: ( "+substr(SM0->M0_TEL,1,3) + ") " + Substr(SM0->M0_TEL,5,10)+' - ('+substr(SM0->M0_FAX,1,3) + ") " + substr(SM0->M0_FAX,5,10),oFont08,100 )
//oPrn:Say( nLin, 1100, "Fax.: ( "+substr(SM0->M0_FAX,1,3) + ")" + substr(SM0->M0_FAX,5,10),oFont08,100 )
oPrn:Say( nLin, 1850, "Nro: "+IS2->C7_NUM,oFont10,100)
//oPrn:Box( nLin, 2650,150,2850 )
//oPrn:Say( nLin, 1980, IS2->C7_NUM,oFont12b,100)
cFecha:=Substr(IS2->C7_EMISSAO,7,2)+"/"+Substr(IS2->C7_EMISSAO,5,2)+"/"+Substr(IS2->C7_EMISSAO,1,4)
oPrn:Say( nLin+50, 1850, "Emisión: "+cFecha,oFont10,100)
//oPrn:Say( nLin+50, 2050, cFecha,oFont12b,100)
//oPrn:Box( nLin, 2900,150,3150 )
nLin:=170     
oPrn:Say( nLin , 0330,  AllTrim( SM0->M0_CEPENT ) + " - C.A.B.A.", oFont08, 100 )
//oPrn:Say( nLin , 0700,  AllTrim( SM0->M0_CEPCOB ) + " -  CIUDAD AUTONOMA DE BUENOS AIRES", oFont08, 100 )


nLin:=250
oPrn:Box( nLin, 0005,nLin+300,1720)
//oPrn:Box( nLin, 0900,nLin+90,1450 )
//oPrn:Box( nLin, 1450,nLin+300,2390 )
oPrn:Box( nLin, 1720,nLin+300,2400)
nLin:=260
...
oPrn:Say( nLin, 0050, SA2->A2_COD + "/" + SA2->A2_LOJA,oFont10,100 )
...
oPrn:Say( nLin, 1750, "Comprador: ",oFont10,100 )
nLin:=300                                         
oPrn:Say( nLin, 1750, BuscaUser(is2->C7_user,1),oFont10,100 )
//oPrn:Box( nLin, 2390,nLin+130,3150 )
oPrn:Say( nLin, 0050, SA2->A2_NOME,oFont10,100 )
...
nLin:=340
oPrn:Say( nLin, 0050, SA2->A2_NREDUZ,oFont10,100 )
//oPrn:Say( nLin, 0920, POSICIONE("CTD",1,xFILIAL("CTD")+IS2->C7_ITEMCTA,"CTD_DESC01"),oFont10,100 )
oPrn:Say( nLin+10, 1750, "Contacto: ",oFont10,100 )
oPrn:Say( nLin+50, 1750, IS2->c7_CONTATO,oFont10,100 )
...
oPrn:Say( nLin, 0050, ALLTRIM( SA2->A2_END ) + " - C.P.: "+SA2->A2_CEP,oFont10,100 )
oPrn:Say( nLin+55, 1750, "Solicitante: ",oFont10,100 )
/*
If !empty(_aLugEnt[3])
...
*/
nLin:=420
oPrn:Say( nLin+55, 1750, BuscaUser(Posicione("SC1",1,Xfilial("SC1")+is2->C7_NUMSC,"C1_SOLICIT"),2),oFont10,100 )
oPrn:Say( nLin, 0050, tABELA("12",SA2->A2_EST),oFont11,100)
...
nLin:=460                          
cTel:="Tel.: "+substr(SA2->A2_XTEL,1,50)+" - Fax.: "+alltrim(SA2->A2_FAX)
oPrn:Say( nLin, 0050, cTel,oFont10,100 )
//oPrn:Say( nLin, 0610, "Fax.: "+alltrim(SA2->A2_FAX),oFont10,100 )
nLin:=500
oPrn:Say( nLin, 0050, "Email.: "+SA2->A2_EMAIL,oFont10,100 )
...
oPrn:Box( nLin, 1625,nLin+50,1860 )
...
oPrn:Box( nLin, 1860,nLin+50,2400 )
...
oPrn:Say( nLin, 1960, "Total",oFont09,070 )
...
oPrn:Say( nLin, 0400, "HOJA ADJUNTA DEL PEDIDO ",oFont10,100)
oPrn:Say( nLin, 1400, "N°",oFont10,100)
oPrn:Box( nLin, 1480,250,1700 )
oPrn:Say( nLin, 1520, cNumPedido,oFont10,100)
nLin:=260
...
AAdd(aRegs,{cPerg,"03","","Imprime OBS?","", "mv_ch3", "N",1,00,0,"C","","mv_par03", "Si", "Si", "Si", "", "", "No", "No", "No", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "","","" } )
