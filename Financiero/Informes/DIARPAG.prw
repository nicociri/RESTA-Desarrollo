...
WHILE !EoF()
	...
	WHILE !EoF() .AND. ORDPAGO == cOrdPago
		...
		IF ANULADO == " " .or. ANULADO == "S"
			...
			ELSEIF TIPODOC $ "GN"
				...
				aNroRet [1]:= right(alltrim(NRODOC),6)
			ELSEIF TIPODOC $ "IV"
				nRETIVA := nRETIVA + VALOR
				aNroRet [2]:= right(alltrim(NRODOC),6)
			ELSEIF TIPODOC $ "IB"
				nRETIB  := nRETIB + VALOR
				aNroRet [3]:= right(alltrim(NRODOC),6)
			ELSEIF TIPODOC $ "SU-SL"
				nRETSUS := nRETSUS + VALOR
				aNroRet [4]:= right(alltrim(NRODOC),6)
	...
	If ( mv_par04 == 1 .OR. ( mv_par04 == 2 .AND. cANULADO == " " ) .OR. ( mv_par04 == 3 .AND. cANULADO == "S" ) ;
		.OR. ( mv_par04 == 4 .AND. cANULADO == "X" ) )
		  if cANULADO<>'S'
		nTOTAL  := nCHQ + nCHQt + nEFE + nTRF + nRETGAN + nRETIVA + nRETIB + nRETSUS
...
If cAnulado != "X"
	...
	//POSICIONE('SA2',1,XFILIAL('SA1')+CCODPROV,"A2_NOM2")
	@ nLine,  34 PSAY substr( POSICIONE('SA2',1,XFILIAL('SA2')+CCODPROV,"A2_NOME"), 1, 29 )
...
@ nLine, 177 PSAY nTotGRA PICTURE "9999999.99" 
nLine := nLine + 1
