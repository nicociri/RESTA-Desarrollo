...
±±³Programa  ³ ARFISF05 ³ Autor ³Microsiga Argentina    ³ Data ³ 14/08/2000 ³±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
 /*/
User Function ARFISF05

PRIVATE aRecSM0Fil :=  ListFil()//Lista las sucursales de la empresa 
...
Private aTipGen  := {}
Private nOpcion  := 0
Private lExtracto:= IIF(GETNEWPAR("PS_SIFEXT","S")=="S",.T.,.F.)
...
MakeDir("C:\TOTVS")
MakeDir("C:\TOTVS\APLICATIVOS")

cPath    := GetNewPar( "MV_RETDIRP", "C:\TOTVS\APLICATIVOS\" )
...
aGenera  := {{"Percepciones/Retenciones IIBB CF"       ,PADR("PER_RET_IIBB_CF_"+cMesAno,50,"")  ,"ProcIBCF()" },; //01   
             ...
             {"Percepcion IVA Clientes "               ,PADR("PER_IVA_CLIENTE_"+cMesAno,50,"")  ,"ProcIVA()"  },; //12
             {"Retenciones IIBB Cobranza"              ,PADR("RET_IIBB_COBRANZA_"+cMesAno,50,""),"RETIIBB()"  },; //13
			 {"SIPRIB(STA FE)"					       ,PADR("SIPRIB_"+cMesAno,50,""),"SIPRIB()"  },; //14
			 {"Retenciones SUSS  "                     ,PADR("RETENCION_SUSS_"+cMesAno,50,"")   ,"ProcSUSS()" }}  //15

For f:=1 to len(aGenera)
     aadd(aGenOpc,aGenera[f,1])
Next f

nPosFil  := Ascan(aRecSM0Fil,{|x| x[2]+x[3]==cEmpAnt+cFilAnt})
cFile    := cFilAnt+aRecSM0Fil[nPosFil][4]+"_"+aGenera[1,2]
cGenera  := aGenera[1,1]
...
If lOk
   ...
   nOpcion := AsCan(aTipGen,{|x| alltrim(x)==alltrim(cTipGen)})
...
±±³Funci¢n   ³ PROCIBCF ³ Autor ³Microsiga Argentina  ³ Data ³14/08/2000³±±
...
If lSigue
   ...
   //levanta la seleccion de sfe totalizada
	...
	cQuery  += "SFE_1.FE_ORDPAGO COMPROB, 'RET' IMPOSTO, '1' TIPOPER, '008' CODNOR, '03' TIPOCOM, '2' TIPODOC, ' ' SERIE, SUM(FE_RETENC) AS RETENC, SUM(FE_VALBASE) AS BASE, 1 AS MOEDA, 1 AS TXMOEDA "
	...
	cQuery  += "SF2.F2_DOC COMPROB, 'PER' IMPOSTO, '2' TIPOPER, '014' CODNOR, '01' TIPOCOM, '2' TIPODOC, SF2.F2_SERIE SERIE, SF2.F2_VALIMP5 AS RETENC, SF2.F2_BASIMP5 AS BASE, SF2.F2_MOEDA AS MOEDA, SF2.F2_TXMOEDA AS TXMOEDA "
	...
	cQuery	+= "(SF2.F2_ESPECIE = 'NF' OR  SF2.F2_ESPECIE = 'NDC') AND "
	...
	cQuery  += "SF1.F1_DOC COMPROB, 'PER' IMPOSTO, '2' TIPOPER, '014' CODNOR, '01' TIPOCOM, '2' TIPODOC, SF1.F1_SERIE SERIE, SF1.F1_VALIMP5 AS RETENC, SF1.F1_BASIMP5 AS BASE, SF1.F1_MOEDA AS MOEDA, SF1.F1_TXMOEDA AS TXMOEDA "
...
//   TCSetField( 'TRB', 'EMISSAO'    , 'D',8, 0 )
	...
	While !Eof()
      ...
      //COTIZACION
      nCotiz:= TRB->TXMOEDA
	  IF TRB->MOEDA != 1 .AND. Empty( nCotiz )
			DbSelectArea( "SM2" )
			DbSeek( TRB->EMISSAO )
			nCotiz := M2_MOEDA2
	  ENDIF
	  IF TRB->MOEDA != 1 .AND. Empty( nCotiz )
			MsgBox( "No existe cotización para el dia: " + DToC( TRB->EMISSAO ), "STOP" )
			nCotiz := 1
	  ENDIF
      
      // VALOR BASE
      cpase 	:= 	str(TRB->BASE * Iif(TRB->MOEDA <> 1, nCotiz, 1),12,2)
      ...
      cpase 	:= 	Str(TRB->RETENC * Iif(TRB->MOEDA <> 1, nCotiz, 1),12,2)
      ...
      If cAlicuota == " 3,00" .and. TRB->IMPOSTO == 'RET'
         cCod_nor  := "016"
      EndIf
      ...
      IF Len(aDet) == 0      
	      AADD(aDet,{ {"Tipo Op.","C" }		 ,{"Cod. Norma","C"}	 ,{"Fecha Retenc.","D" },;
	                  {"Tip.Comprob. ","C" },{"Letra","C"}  		 ,{"Comprobrante","C"}   	,;
	                  {"Fec.Comprob.","D" }	 ,{"Importe ","N"}		 ,{"Nro Certif.","C"}	,;
	                  {"Tipo Doc.","C"}   	 ,{"Documento","C"}  	 ,{"Sit.IB","C"}		,;
	                  {"Nro Insc.","C"}	 	 ,{"Sit.IVA","C"}  	 	 ,{"Razon Social","C"}	,; 
	                  {" ","C"}  	 		 ,{" ","C"}	         	 ,{"Monto Neto","N"}	,;                                                  
	                  {"Alic.","N"}		 	 ,{"Retenc.","N"}	     ,{"Total Retenc.","N"}} )           
...
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descrip.  ³ FUNCION admin	OBTIENE SITUACION IVA                              ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Uso       ³ RETF010                                                    ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³  Fecha   ³ Programador   ³ Alteraci¢n Efectuada                       ³±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funci¢n   ³ PROCSUSS ³ Autor ³Microsiga Argentina  ³ Data ³14/08/2000³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
IF nOpcion == 1 .or. nOpcion== 3
    ...
    For u:=1 to len(aConSUSS)
	 	cArSUSS       := Alltrim(cPath)  + SUBSTR(Alltrim(cFile),1,len(Alltrim(cFile))-4)+ "_"+aConSUSS[u,6]+"_"+aConSUSS[u,3]+".txt" 
	 	aConSUSS[u,4] := cArSUSS
		While ( nPuntRet := FCreate(cArSUSS ) ) == -1
		   If !MsgYesNo("No se puede Crear el Archivo "+cArSifR+Chr(13)+Chr(10)+"Continua?")
				lSigue   := .F.
				Exit
			EndIf
		EndDo 
	 	aConSUSS[u,5] := nPuntRet
	Next u                       
...
If lSigue
   ...
   //-----------------------------------------------------------------------------------------------
   //   RETENCIONES DE SUSS CON EL CONCEPTO INFORMADO
   //
   //------------------------------------------------------------------------------------------------    

  	cQuery    := "SELECT SFF.FF_CFO CODREG, SFE_1.FE_NROCERT CERTIF, SFE_1.FE_EMISSAO EMISSAO, SFE_1.FE_FORNECE FORNECE, SFE_1.FE_LOJA LOJA, "
  	cQuery    += " SA2.A2_CGC CGC, SA2.A2_NOME NOME, SFE_1.FE_TIPO TIPO, SFE_1.FE_ORDPAGO OPAGO, "
	...
	cQuery 	  += "     SFE_2.FE_ORDPAGO = SFE_1.FE_ORDPAGO AND "
	cQuery 	  += "     SFE_2.FE_FORNECE = SFE_1.FE_FORNECE AND "
	cQuery 	  += "     SFE_2.FE_LOJA = SFE_1.FE_LOJA AND "
	cQuery    += "     SFE_2.FE_DTESTOR=SFE_1.FE_DTESTOR AND "
	cQuery    += "     SFE_2.FE_DTRETOR=SFE_1.FE_DTRETOR "
	cQuery    += " ) AS RETENC, "
	cQuery    += " SFE_1.FE_CONCEPT FE_CONCEPT, SFE_1.FE_DTRETOR,SFE_1.FE_DTESTOR "
	cQuery    += " FROM " + RETSQLNAME ('SFE') + " SFE_1 "
	...
	cQuery	  += "SFE_1.FE_EMISSAO <= '" + DTOS(mv_par02)+ "' AND "
    cQuery    += "( ( SFE_1.FE_DTESTOR = '' ) OR "
    cQuery    += "  (SFE_1.FE_DTESTOR > '" + DTOS(mv_par02)+ "' ) OR "
    cQuery    += "  (SFE_1.FE_DTRETOR <> ' ' AND ( SFE_1.FE_DTRETOR < '"+DTOS(MV_PAR01)+"') ) "
    cQuery    += " ) AND "
    cQuery	  += "SFF.FF_IMPOSTO = 'SUS' AND "
    cQuery    += "SFF.D_E_L_E_T_ <> '*' AND "
    cQuery    += "SA2.D_E_L_E_T_ <> '*' AND "
	cQuery    += "SFE_1.D_E_L_E_T_ <> '*' "
	cQuery	  += "GROUP BY SFF.FF_CFO , SFE_1.FE_NROCERT, SFE_1.FE_EMISSAO, SFE_1.FE_FORNECE, SFE_1.FE_LOJA, SA2.A2_NOME,SA2.A2_CGC, SFE_1.FE_TIPO, SFE_1.FE_ORDPAGO,SFE_1.FE_CONCEPT, SFE_1.FE_DTRETOR,SFE_1.FE_DTESTOR "
	...
	While !Eof()
	  ...
	  //	IncRegua()
		...
		//----------------------------           
		IF TRB->RETENC > 0 ////"00000000000"
			cExed		:= "00000000.00"
			cExed       := RIGHT(StrTran( cExed, ".", "0" ),11) ////"00000000000"
			cImpRet  	:= Str( TRB->RETENC, 14, 2 )
			cImpRet  	:= RIGHT(StrTran( cImpRet, ".", "," ),11)
		ELSE
			cExed		:= Str( TRB->RETENC*-1, 14, 2 )
			cExed       := RIGHT(StrTran( cExed, ".", "," ),11)
			cImpRet  	:= "00000000.00"
			cImpRet  	:= RIGHT(StrTran( cImpRet, ".", "0" ),11) ////"00000000000"
		ENDIF
		//----------------------------
...
//		cNroCOri 	:= RIGHT ("00000000000000" + Alltrim(TRB->CERTIF),14)
      ...
      */
        ...
        //Genera archivo o ambos
        nPosSUS:=AsCan(aConSUSS,{|x| x[1]==TRB->FE_CONCEPT } )
        IF nOpcion == 1 .or. nOpcion== 3
		   FWrite(  aConSUSS[nPosSUS,5],  cString, Len(cString)  )
        ...
        IF nOpcion == 2 .or. nOpcion== 3
		    IF Len(aDet) == 0      
			      AADD(aDet,{ {"Cuit Proveedor","C"} ,{"Codigo / Tienda","C"},{PADR("Razon Social",50),"C"}	, 	{"Fecha retencion","C"} 	, ;
			                  {"Numero comprobante","C"} 	, {"Importe retencion","N"},{"Tipo Comprobante      .","C"}, {"Importe exedente","N"}} )
	      
	        ENDIF      
           AADD(aDet,{ cNroCUIT,TRB->FORNECE+"-"+TRB->LOJA,TRB->NOME,cFechRet,TRB->CERTIF,StrTran( cImpRet, ",", "." ),aConSUSS[nPosSUS,3],StrTran( cExed, ",", "." )/*cTipComp*/})
      ENDIF
		DbSkip()
  ...
  //--------------------------------------------------------------------------------------------/
  //
  // aGREGAR SEGUNDA QUERY QUE EJECUTE LAS RETENCIONES DE SUSS ANULADAS 
  //--------------------------------------------------------------------------------------------/
	cQuery := "SELECT SFF.FF_CFO CODREG, SFE_1.FE_NROCERT CERTIF, SE2.E2_EMISSAO EMISSAO, SFE_1.FE_FORNECE FORNECE, SFE_1.FE_LOJA LOJA, SA2.A2_CGC CGC,"
	cQuery += " SA2.A2_NOME NOME, SFE_1.FE_TIPO TIPO, SFE_1.FE_ORDPAGO OPAGO, "
	cQuery += "(SE2.E2_VALOR * -1) AS RETENC, SFE_1.FE_CONCEPT FE_CONCEPT  "
	cQuery += "FROM "+RetSqlName("SE2")+" SE2 "
	cQuery += "INNER JOIN "+RetSqlName("SFE")+" SFE_1 "
	cQuery += "    ON SFE_1.FE_ORDPAGO=SE2.E2_ORDPAGO AND SFE_1.FE_NROCERT<>'NORET' AND SFE_1.FE_TIPO='S'"
	//cQuery += "    ON SFE_1.FE_ORDPAGO=SE2.E2_PSOPORI AND SFE_1.FE_NROCERT=SE2.E2_PSCEORI AND SFE_1.FE_TIPO='S'"//
	cQuery += "       AND SFE_1.D_E_L_E_T_=''  "
    cQuery += "LEFT JOIN "+RETSQLNAME('SFF')+" SFF ON SFE_1.FE_CONCEPT = SFF.FF_ITEM AND SFE_1.FE_ALIQ = SFF.FF_ALIQ "
	cQuery += "LEFT JOIN "+RetSqlName("SA2")+" SA2 "
	cQuery += "    ON SFE_1.FE_FORNECE = SA2.A2_COD AND SFE_1.FE_LOJA = SA2.A2_LOJA "
	cQuery += "WHERE "
	cQuery += "SE2.E2_FILIAL ='"+xFilial("SE2")+"' AND "
	cQuery += "SE2.D_E_L_E_T_='' AND "
	cQuery += "( SE2.E2_EMISSAO BETWEEN '"+DTOS(MV_PAR01)+"' AND '"+DTOS(MV_PAR02)+"'  ) AND "
	cQuery += "SE2.E2_TIPO='RSD' AND "
	cQuery += "SFE_1.FE_FILIAL = '"+xFilial("SFE")+"' AND "
	cQuery += "SFE_1.FE_TIPO='S' AND "
	cQuery += "SFE_1.D_E_L_E_T_ <> '*' AND "
	cQuery += "SA2.A2_FILIAL = '  ' AND "
	cQuery += "SA2.D_E_L_E_T_ <> '*' AND "
    cQuery += "SFF.FF_IMPOSTO = 'SUS' AND "
    cQuery += "SFF.D_E_L_E_T_ <> '*' "
	cQuery += "GROUP BY SFF.FF_CFO, SFE_1.FE_NROCERT, SE2.E2_VALOR ,SFE_1.FE_EMISSAO, SFE_1.FE_FORNECE, SFE_1.FE_LOJA, "
	cQuery += "SA2.A2_CGC,SA2.A2_NOME, SFE_1.FE_TIPO, SFE_1.FE_ORDPAGO, SFE_1.FE_CONCEPT, SE2.E2_EMISSAO   "
...
//		cNroCOri 	:= RIGHT ("00000000000000" + Alltrim(TRB->CERTIF),14)
      ...
      */
        ...
        //Genera archivo o ambos
        nPosSUS:=AsCan(aConSUSS,{|x| x[1]==TRB->FE_CONCEPT } )
        IF ( nOpcion == 1 .or. nOpcion== 3 ) .and. nPosSUS > 0
//		   FWrite(  nPuntero,  cString, Len(cString)  )
		   FWrite(  aConSUSS[nPosSUS,5],  cString, Len(cString)  )
        ENDIF          
  ...
  //--------------------------------------------------------------------------------------------/
  	...
  	cQuery   := "SELECT SFE_1.FE_NROCERT CERTIF, SFE_1.FE_EMISSAO EMISSAO, SFE_1.FE_FORNECE FORNECE, SFE_1.FE_LOJA LOJA, "
  	cQuery   += "SA2.A2_CGC CGC,SA2.A2_NOME NOME, SFE_1.FE_TIPO TIPO, SFE_1.FE_ORDPAGO OPAGO, "
	cQuery   += "(SELECT SUM(FE_RETENC) FROM " + RETSQLNAME ('SFE') + " SFE_2 "
	cQuery   += "   WHERE "
	cQuery   += "     SFE_2.FE_TIPO='S' 		AND "
	...
	cQuery   += "     SFE_2.FE_DTESTOR = SFE_1.FE_DTESTOR AND "
	cQuery   += "     SFE_2.FE_DTRETOR = SFE_1.FE_DTRETOR "
	cQuery   += " ) AS RETENC, SFE_1.FE_DTRETOR, SFE_1.FE_DTESTOR "
	cQuery   += " FROM " + RETSQLNAME ('SFE') + " SFE_1 "
    cQuery	 += "LEFT JOIN " + RETSQLNAME ('SA2') + " SA2 ON SFE_1.FE_FORNECE = SA2.A2_COD AND SFE_1.FE_LOJA = SA2.A2_LOJA "
    cQuery   += "WHERE "
    cQuery	 += "SFE_1.FE_FILIAL = '" + xFilial("SFE") +	"' AND "
    ...
    cQuery   += "SA2.D_E_L_E_T_ <> '*' AND "
	cQuery   += "SFE_1.D_E_L_E_T_ <> '*' "
	cQuery	 += "GROUP BY SFE_1.FE_NROCERT, SFE_1.FE_EMISSAO, SFE_1.FE_FORNECE, SFE_1.FE_LOJA, SA2.A2_CGC,SA2.A2_NOME, SFE_1.FE_TIPO, SFE_1.FE_ORDPAGO, SFE_1.FE_DTRETOR,SFE_1.FE_DTESTOR "
...
//		cNroCOri 	:= RIGHT ("00000000000000" + Alltrim(TRB->CERTIF),14)
      ...
      */
        ...
        //Genera archivo o ambos
        nPosSUS:=AsCan(aConSUSS,{|x| x[1]=="5 " } )
        IF nOpcion == 1 .or. nOpcion== 3
//		   FWrite(  nPuntero,  cString, Len(cString)  )
		   FWrite(  aConSUSS[nPosSUS,5],  cString, Len(cString)  )
        ENDIF          
   ...
   lOK := .F.
   IF nOpcion == 1 .or. nOpcion== 3 
       For h:=1 to len(aConSUSS) 
		   If FCLOSE( aConSUSS[h,5] )
		      lOK := .T.
		   Else
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funci¢n   ³ PROCIBR  ³ Autor ³Microsiga Argentina  ³ Data ³14/08/2000³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
If lSigue
   ...
   While !eof()
      ...
      If ALLTRIM(FE_TIPO) <> "B"   .or. ( ALLTRIM(FE_TIPO) = "B" .and.  AllTrim(FE_EST)!="BA") .or. Empty(FE_FORNECE)
         DbSkip()
         Loop
      EndIf
     ...
     End
      ...
      /*/------------Estructura a pasar-----------------------------------//
         ...
         Importe Retenido   		:= ''    36      46    Decimal       11
         Tipo de Operacion  		:= ''    47      47    Texto         01

      */
	  
	  cString  := cCUITFor + cFecha + cNroSuc + cNroEmis + cRet+ "A" + Chr(13) + Chr(10)
      ...
      IF nOpcion == 2 .or. nOpcion== 3
	      IF Len(aDet) == 0      
		      AADD(aDet,{ {"Cuit Proveedor","C"} ,{"Codigo / Tienda","C"},{PADR("Razon Social",50),"C"}	, 	{"Fecha retencion","C"} 	, ;
		                 {"Nro Sucursal","C"}	,	{"Numero comprobante","C"} 	, {"Importe retencion","N"}})
	      
	      ENDIF      
	      AADD(aDet,{ cCUITFor ,SA2->A2_COD+"-"+SA2->A2_LOJA, UPPER(SA2->A2_NOME),cFecha , cNroSuc , cNroEmis , cRet  })
      ENDIF
   EndDo
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funci¢n   ³          ³ Autor ³Microsiga Argentina  ³ Data ³      2000³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descrip.  ³                                                            ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Uso       ³                                                            ³±±
            ...
            */
			...
			/*/------------Estructura a pasar-----------------------------------//
         	...
         	monto Imponible  			:= ''    38      49    Decimal       12
           	...
           	Tipo de operacion  			:= ''    61      61    Texto         01
...
//         cCUIT    := SubStr(posicione("SA2",1,cFilSA2+TMP->(F2_CLIENTE+F2_LOJA),"A2_CGC"),1,13)
		    ...
		    If Alltrim(TMP->F2_ESPECIE) =="CF"
				...
				nBasSL1IB   := SL1->L1_XBASINT
				nValSL1IB   := SL1->L1_XIBINT
			ENDIF
            //---------------------------------------------------
...
LOCAL cTipoDoc 	:= "80"
LOCAL cCodImp  	:= "767"
LOCAL cCodReg	:= "493"   // PARA PERCEPCIONES A NO CATEGORIZADOS EL REGIMEN ES 270
LOCAL cCodOp	:= "2"
LOCAL cCodCon 	:= "00"
Local aDet      := {}
LOCAL nCotiz	:= 0
...
If lSigue
	...
	cQuery := "SELECT F1_FORNECE, F1_LOJA, F1_EMISSAO, F1_DTDIGIT, F1_DOC, F1_ESPECIE, F1_SERIE, F1_VALIMP4, F1_BASIMP4 , F1_VALBRUT, F1_MOEDA, F1_TXMOEDA "
	...
	While !('TMP')->(EOF())
		IncProc()
		If TMP->F1_VALIMP4 != 0
		nCotiz := TMP->F1_TXMOEDA
				IF TMP->F1_MOEDA != 1 .AND. Empty( nCotiz )
					DbSelectArea( "SM2" )
					DbSeek( TMP->F1_EMISSAO )
					nCotiz := M2_MOEDA2
				ENDIF
				IF TMP->F1_MOEDA != 1 .AND. Empty( nCotiz )
					MsgBox( "No existe cotización para el dia: " + DToC( TMP->F1_EMISSAO ), "STOP" )
					nCotiz := 1
				ENDIF

			cNroCUIT  	:= PadR(Alltrim(posicione("SA1",1,cFilSA1+TMP->(F1_FORNECE+F1_LOJA),"A1_CGC")),20)
			// valor percep.
			cImpRet	:=Str( TMP->F1_VALIMP4*nCotiz, 14 )
			cImpRet := Transform(TMP->F1_VALIMP4*nCotiz,'@E 99999999999.99')
			StrTran( cImpRet, ".", "," )
			// monto Base
			cBase	:=Str( TMP->F1_BASIMP4*nCotiz, 14 )
			cBase 	:= Transform(TMP->F1_BASIMP4*nCotiz,'@E 99999999999.99')
			StrTran( cBase, ".", "," )
			// monto Bruto
			cNeto	:=Str( TMP->F1_VALBRUT*nCotiz, 16 )
			cNeto 	:= Transform(TMP->F1_VALBRUT*nCotiz,'@E 9999999999999.99')
			StrTran( cNeto, ".", "," )
			// Tipo Comprobante
			If TMP->F1_ESPECIE = 'NF'
					cTipComp := '01'
	...
	cQuery := "SELECT F2_CLIENTE, F2_LOJA, F2_EMISSAO, F2_DTDIGIT, F2_DOC, F2_ESPECIE, F2_SERIE, F2_VALIMP4 , F2_VALIMPS, F2_VALIMPU, F2_BASIMP4 , F2_BASIMPS ,F2_BASIMPU , F2_VALBRUT, F2_MOEDA, F2_TXMOEDA "
	cQuery += "FROM " + RetSQLName ("SF2") +" WHERE"
    cQuery += " (F2_EMISSAO >= '" + DTOS(mv_par01) + "' AND F2_EMISSAO <='" + DTOS(mv_par02)+"')"
	cQuery += " AND (F2_VALIMP4 <> 0 OR F2_VALIMPS <> 0 OR F2_VALIMPU <> 0) AND (D_E_L_E_T_ <> '*') AND (F2_ESPECIE = 'NF ' OR F2_ESPECIE = 'NDC') "
	...
	While !('TMP')->(EOF())
		IncProc()
		If TMP->F2_VALIMP4 != 0 .or.TMP->F2_VALIMPS != 0.or.TMP->F2_VALIMPU != 0
			nCotiz := TMP->F2_TXMOEDA
				IF TMP->F2_MOEDA != 1 .AND. Empty( nCotiz )
					DbSelectArea( "SM2" )
					DbSeek( TMP->F2_EMISSAO )
					nCotiz := M2_MOEDA2
				ENDIF
				IF TMP->F2_MOEDA != 1 .AND. Empty( nCotiz )
					MsgBox( "No existe cotización para el dia: " + DToC( TMP->F2_EMISSAO ), "STOP" )
					nCotiz := 1
				ENDIF

			cNroCUIT  	:= PadR(Alltrim(posicione("SA1",1,cFilSA1+TMP->(F2_CLIENTE+F2_LOJA),"A1_CGC")),20)
			// valor percep.
			cImpRet	:=Str( TMP->F2_VALIMP4*nCotiz, 14 )
			cImpRet := Transform(TMP->F2_VALIMP4*nCotiz,'@E 99999999999.99')
			// monto Base
     		If TMP->F2_VALIMPS != 0.or.TMP->F2_VALIMPU != 0
 			   cImpRet	:=Str( (TMP->F2_VALIMPS+TMP->F2_VALIMPU)*nCotiz, 14 )
			   cImpRet  := Transform((TMP->F2_VALIMPS+TMP->F2_VALIMPU),'@E 99999999999.99')
  			   cBase	:=Str( (TMP->F2_BASIMPS + TMP->F2_BASIMPU)*nCotiz, 14 )
			   cBase 	:= Transform((TMP->F2_BASIMPS + TMP->F2_BASIMPU)*nCotiz,'@E 99999999999.99')
			   cCodReg	:= "270"
     		ELSE
 			   cImpRet	:=Str( TMP->F2_VALIMP4*nCotiz, 14 )
			   cImpRet := Transform(TMP->F2_VALIMP4*nCotiz,'@E 99999999999.99')
   			   cBase	:=Str( TMP->F2_BASIMP4*nCotiz, 14 )            
			   cBase 	:= Transform(TMP->F2_BASIMP4*nCotiz,'@E 99999999999.99')
			   cCodReg	:= "493"
			ENDIF
			StrTran( cImpRet, ".", "," )
			StrTran( cBase, ".", "," )
			// monto Bruto
			cNeto	:=Str( TMP->F2_VALBRUT*nCotiz, 16 )
			cNeto 	:= Transform(TMP->F2_VALBRUT*nCotiz,'@E 9999999999999.99')
			StrTran( cNeto, ".", "," )
			// Tipo Comprobante
            ...
            //Genera archivo o ambos
            IF nOpcion == 2 .or. nOpcion== 3
               AADD(aDet,{ TMP->F2_ESPECIE,cFechaDoc,TMP->F2_DOC,TMP->F2_VALBRUT*nCotiz,TMP->F2_BASIMP4*nCotiz,TMP->F2_VALIMP4*nCotiz,cNroCUIT })
			ENDIF
...
aAdd(aPergs,{'De Certificado ?','De Certificado ?','De Certificado ?','mv_ch3','C', 12, 0, 0, 'G', '', 'mv_par03','','','','','','','','','','','','','','','','','','','','','','','','','',''})
aAdd(aPergs,{'A  Certificado ?','A  Certificado ?','A  Certificado ?','mv_ch4','C', 12, 0, 0, 'G', '', 'mv_par04','','','','','','','','','','','','','','','','','','','','','','','','','',''})
...
If lSigue
	...
	cQuery += " F1_XEXTRAC <> '1' AND F1_ESPECIE IN ('NF','NDP') "      
	...
	While !('TMP')->(EOF())
		...
		If .T.
            ...
            cDespacho:= PADL(TMP->F1_XDESIMP,20," ")
	...
	cQuery += " (F2_DTDIGIT >= '" + DTOS(mv_par01) + "' AND F2_DTDIGIT <='" + DTOS(mv_par02)+"') AND "
	cQuery += " F2_XEXTRAC <> '1' AND F2_ESPECIE = 'NCP' "
	cQuery += " AND D_E_L_E_T_<>'*' "
	cQuery += " ORDER BY F2_DTDIGIT"
	...
	While !('TMP')->(EOF())
		...
		If .T.
            ...
            For k:=1 to len(aJuris)
                cCampo:="TMP->F2_VALIMP"+aJuris[k,1]
				If &(cCampo) != 0
					cCod     := aJuris[k,2]//codigo jurisdiccion "901"
					nImporte := &(cCampo) * IIF(TMP->F2_MOEDA > 1, TMP->F2_TXMOEDA, 1 )
					nImp:=Str( nImporte, 14 )
					nImp := Transform(nImporte,'@E 99999999.99')
					StrTran( nImp, ".", "," )
					cString := cCod + cCUIT + cFecha + cSuc + cConst + cTipo + cLetra + nImp + Chr(13) + Chr(10)
		...
		If .T.
            ...
            For k:=1 to len(aJuris)
				...
				If &(cCampo) != 0
					...
					nImporte := &(cCampo) * IIF(TMP->F1_MOEDA > 1, TMP->F1_TXMOEDA, 1 )
					nImp:=Str( nImporte, 14 )
					nImp := Transform(nImporte,'@E 9999999.99')
					StrTran( nImp, ".", "," )             
					
					cString := cCod + cCUIT + cFecha + "0000000000000000000000" + "CC" + IIF(TMP->F1_MOEDA > 1, 'E', 'P' ) +nImp + Chr(13) + Chr(10)
	                //Genera archivo o ambos
	                IF nOpcion == 1 .or. nOpcion== 3
	...
	While !('TMP')->(EOF())
		IncProc()
   		If TMP->EL_VLMOED1 != 0
			...
			If TMP->EL_ZONFIS == "CF"        // Ciudad de Buenos Aires
            	cCod:= "901"
         	ElseIf TMP->EL_ZONFIS == "BA"    // Provincia de Buenos Aires
            	cCod := "902"
         	ElseIf TMP->EL_ZONFIS == "CA"    // Catamarca
         		cCod := "903"
         	ElseIf TMP->EL_ZONFIS == "CO"    // Cordoba
            	cCod := "904"
         	ElseIf TMP->EL_ZONFIS == "CR"    // Corrientes
            	cCod := "905"
         	ElseIf TMP->EL_ZONFIS == "CH"    // Chaco
            	cCod := "906"
         	ElseIf TMP->EL_ZONFIS == "CB"    // Chubut
            	cCod := "907"
         	ElseIf TMP->EL_ZONFIS == "ER"    // Entre Rios
            	cCod := "908"
         	ElseIf TMP->EL_ZONFIS == "FO"    // Formosa
            	cCod := "909"
         	ElseIf TMP->EL_ZONFIS == "JU"    // Jujuy
            	cCod := "910"
         	ElseIf TMP->EL_ZONFIS == "LP"    // La Pampa
            	cCod := "911"
         	ElseIf TMP->EL_ZONFIS == "LR"    // La Rioja
            	cCod := "912"
         	ElseIf TMP->EL_ZONFIS == "ME"    // Mendoza
            	cCod := "913"
         	ElseIf TMP->EL_ZONFIS == "MI"    // Misiones
            	cCod := "914"
         	ElseIf TMP->EL_ZONFIS == "NE"    // Neuquen
            	cCod := "915"
         	ElseIf TMP->EL_ZONFIS == "RN"    // Rio Negro
            	cCod := "916"
         	ElseIf TMP->EL_ZONFIS == "SA"    // Salta
            	cCod := "917"
         	ElseIf TMP->EL_ZONFIS == "SJ"    // San Juan
            	cCod := "918"
         	ElseIf TMP->EL_ZONFIS == "SL"    // San Luis
            	cCod := "919"
         	ElseIf TMP->EL_ZONFIS == "SC"    // Santa Cruz
            	cCod := "920"
         	ElseIf TMP->EL_ZONFIS == "SF"    // Santa Fe
            	cCod := "921"
         	ElseIf TMP->EL_ZONFIS == "SE"    // Santiago del Estero
            	cCod := "922"
         	ElseIf TMP->EL_ZONFIS == "TF"    // Tierra del Fuego
            	cCod := "923"
         	ElseIf TMP->EL_ZONFIS == "TU"    // Tucuman
            	cCod := "924"
         	Else
            	cCod := "000" // Codigo erroneo. Solo para que no se alteren las posiciones
         	EndIf
...
//          cFecha   := SUBSTR(TRB->F1_DTDIGIT,7,2)+"/"+SUBSTR(TRB->F1_DTDIGIT,5,2)+"/"+SUBSTR(TRB->F1_DTDIGIT,1,4)
   ...
   While !TRB1->(EOF())
            ...
            //Se posiciona en el proveedor
            SA2->(DbSeek(XFILIAL("SA2")+TRB1->F2_CLIENTE+TRB1->F2_LOJA))
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funci¢n   ³ PROCIBB  ³ Autor ³ms					    ³ Data ³03/11/2010³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
//   cFile    := aGenera[nPosOpc,2]
   nPosFil  := Ascan(aRecSM0Fil,{|x| x[2]+x[3]==cEmpAnt+cFilAnt})
   cFile    := cFilAnt+aRecSM0Fil[nPosFil][4]+"_"+aGenera[nPosOpc,2]
ENDIF  
...
±±³Funcion   ³ ARFISR01 ³ Autor ³ MS				    ³ Data ³ 11/05/10  ³±±
...
For g:=1 to len(aDetalle[1])
    ...
    IF aDetalle[1,g,2]=="N"
       oSection1:Cell(aDetalle[1,g,1]):SetPicture("@E 999999999.99")
...
For h:=2 to len(aDetalle)
    ...
    For j:=1 to len(aDetalle[h])                             
        ...
        xValor:= aDetalle[h,j]
        IF aDetalle[1,j,2]=="N"
           xValor:= VAL(StrTran( aDetalle[h,j], ",", "." ))
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³xCONSUSS  ºAutor  ³Microsiga           ºFecha ³  02/16/11   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³obtiene conceptos de SUSS                                   º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ AP                                                         º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
...
*/              
//aConSUSS := xConSUSS()
Static Function xConSUSS

Local aConSUSS:={}


aConSUSS:={{ "1 ","RG 1769 - SERVICIOS DE SEGURIDAD"            ,"RG 1769-SERVICIOS DE SEGURIDAD","",0,"754"},;
           { "2 ","RG 1784 - GENERAL DE CONTRIBUCIONES"         ,"RG 1784-GENERAL DE CONTRIBUCIONES","",0,"755"},;
           { "3 ","NO USAR - RG 4052 DEROGADA POR RG 2682"      ,"NO USAR-RG 4052 DEROGADA P","",0,""},;
           { "5 ","RG 3983 - SERVICIOS EVENTUALES"              ,"RG 3983-SERVICIOS EVENTUALES","",0,"742"},;
           { "4 ","RG 1556 - SERVICIOS DE LIMPIEZA EN INMUEBLES","RG 1556-SERVICIOS DE LIMPIEZA","",0,"748"},;
           { "6 ","RG 2682 - EMPRESAS CONSTRUCTORAS, INGENIERIA Y ARQUITECTURA","RG 2682-EMPRESAS CONSTRUCTORAS","",0,"740"}}
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funci¢n   ³ SIPRIB ³ Autor ³Microsiga Argentina  ³ Data   ³14/08/2000  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descrip.  ³ Genera Archivo de Retenciones     de IIBB -SF              ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Uso       ³ RETF010                                                    ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³  Fecha   ³ Programador   ³ Alteraci¢n Efectuada                       ³±±
...
If lSigue
   ...
   //levanta la seleccion de sfe totalizada
	cQuery  := "SELECT SFE_1.FE_NROCERT CERTIF, SFE_1.FE_EMISSAO EMISSAO, SFE_1.FE_FORNECE FORCLI, SFE_1.FE_LOJA LOJA, SA2.A2_CGC CGC, SA2.A2_ISICM CONVENIO, SFE_1.FE_TIPO TIPO, SFE_1.FE_ALIQ ALIC, "
	cQuery  += "SFE_1.FE_ORDPAGO COMPROB, 'RET' IMPOSTO, '1' TIPOPER, '020' CODNOR, '03' TIPOCOM, '3' TIPODOC, ' ' SERIE, SUM(FE_RETENC) AS RETENC, SUM(FE_VALBASE) AS BASE, 1 AS MOEDA, 1 AS TXMOEDA "
    cQuery	+= "FROM " + RETSQLNAME ('SFE') + " SFE_1 "
    cQuery	+= "LEFT JOIN " + RETSQLNAME ('SA2') + " SA2 ON SFE_1.FE_FORNECE = SA2.A2_COD AND SFE_1.FE_LOJA = SA2.A2_LOJA "
    cQuery  += "WHERE "
    cQuery	+= "SFE_1.FE_FILIAL = '" 	+ xFilial("SFE") +	"' AND "
	...
	cQuery	+= "UNION ALL "
	//<<<<<< percepcion NF-NDC >>>>>>>>
    cQuery  += "SELECT ' ' CERTIF, SF2.F2_EMISSAO EMISSAO, SF2.F2_CLIENTE FORCLI, SF2.F2_LOJA LOJA, SA1.A1_CGC CGC, SA1.A1_ISICM CONVENIO, SF2.F2_ESPECIE TIPO, 2.50 AS ALIC, "
	cQuery  += "SF2.F2_DOC COMPROB, 'PER' IMPOSTO, '2' TIPOPER, '029' CODNOR, '01' TIPOCOM, '3' TIPODOC, SF2.F2_SERIE SERIE, SF2.F2_VALIMPK AS RETENC, SF2.F2_BASIMPK AS BASE, SF2.F2_MOEDA AS MOEDA, SF2.F2_TXMOEDA AS TXMOEDA "
    cQuery	+=	"FROM " + RETSQLNAME ('SF2') + " SF2 "
    cQuery	+= "LEFT JOIN " + RETSQLNAME ('SA1') + " SA1 ON SF2.F2_CLIENTE = SA1.A1_COD AND SF2.F2_LOJA = SA1.A1_LOJA "
    cQuery  += "WHERE "
    cQuery	+= "SF2.F2_FILIAL = '" + xFilial("SF2") +	"' AND "
	cQuery	+= "SA1.A1_FILIAL = '" + xFilial("SA1") + "' AND "
	cQuery  += "SF2.F2_VALIMPK > 0 AND "
	cQuery	+= "(SF2.F2_ESPECIE = 'NF' OR  SF2.F2_ESPECIE = 'NDC') AND "
	cQuery	+= "SF2.F2_EMISSAO >= '" + DTOS(mv_par01)+ "' AND "
	cQuery	+= "SF2.F2_EMISSAO <= '" + DTOS(mv_par02)+ "' AND "
	cQuery  += "SF2.D_E_L_E_T_ <> '*' "
	//<<<<<< percepcion NCC >>>>>>>>
	cQuery	+= "UNION ALL "
    cQuery  += "SELECT ' ' CERTIF, SF1.F1_EMISSAO EMISSAO, SF1.F1_FORNECE FORCLI, SF1.F1_LOJA LOJA, SA1.A1_CGC CGC, SA1.A1_ISICM CONVENIO, SF1.F1_ESPECIE TIPO, 2.50 AS ALIC, "
	cQuery  += "SF1.F1_DOC COMPROB, 'PER' IMPOSTO, '2' TIPOPER, '029' CODNOR, '09' TIPOCOM, '3' TIPODOC, SF1.F1_SERIE SERIE, SF1.F1_VALIMPK AS RETENC, SF1.F1_BASIMPK AS BASE, SF1.F1_MOEDA AS MOEDA, SF1.F1_TXMOEDA AS TXMOEDA "
    cQuery	+=	"FROM " + RETSQLNAME ('SF1') + " SF1 "
    cQuery	+= "LEFT JOIN " + RETSQLNAME ('SA1') + " SA1 ON SF1.F1_FORNECE = SA1.A1_COD AND SF1.F1_LOJA = SA1.A1_LOJA "
    cQuery  += "WHERE "
    cQuery	+= "SF1.F1_FILIAL = '" + xFilial("SF1") +	"' AND "
	cQuery	+= "SA1.A1_FILIAL = '" + xFilial("SA1") + "' AND "
	cQuery  += "SF1.F1_VALIMPK > 0 AND "
	cQuery	+= "SF1.F1_ESPECIE = 'NCC' AND "
	cQuery	+= "SF1.F1_EMISSAO >= '" + DTOS(mv_par01)+ "' AND "
	cQuery	+= "SF1.F1_EMISSAO <= '" + DTOS(mv_par02)+ "' AND "
	cQuery  += "SF1.D_E_L_E_T_ <> '*' "
...
//   TCSetField( 'TRB', 'EMISSAO'    , 'D',8, 0 )
	...
	While !Eof()
      ...
      Tipo de Operacion     1  - 1   Numerico         1					2
      F. de Retencion       2  - 11  Fecha
      Codigo de Norma       12 - 14  TEXTO     		020 				029      
      T. de Comprobante     15 - 16  Numerico        03					01(NF)-02(ND)-09(NC)
      Letra de Comprobante  17 - 17  Texto
      Número de Doc.        18 - 33  Texto           16                 16
      Fecha de Comprobante  34 - 43  Fecha
      Monto                 44 - 55  Numérico 9-2
      Tip. Doc.             56 - 56  Numerico         3					3
      Nro certificado       57 - 67  Texto
      Sit  IB               68 - 68  Numerico
      Nro. Insc. IIBB       69 - 78  Numerico
      Sit  IVA              79 - 79  Numerico
      marca inscr o.grav.   80 - 80  texto           0                  0
      marca inscr dRel	    81 - 81  texto           0                  0
      Importe otros grav.   82 - 93  numerico 9-2
      Iva                   94 -105  Numerico 9-2
      Monto base a Ret.    106 -117  Numerico 9-2
      Alicuota             118 -122  Numerico 2-2
      impusto determ.	   123- 134	 Numerico 9-2  idem monto retenido
      derecho registro	   135 -146  Numerico 9-2    0                  0
      Ret. / Perc. Prac    147 -158  Numerico 9-2
      artic/inciso p/cal   159 -161  texto      perce (009);retenc c/cm (002); retenc s/cm (003) 
      ...
      cCod_nor 	:= 	TRB->CODNOR
      cFecper  	:= 	SubStr(TRB->EMISSAO,7,2) + '/' + SubStr(TRB->EMISSAO,5,2) + '/' +  SubStr(TRB->EMISSAO,1,4)
      cTipocom 	:= 	IIF ( TRB->IMPOSTO='RET',TRB->TIPOCOM,IIF(TRB->TIPO='NDC','02',IIF(TRB->TIPO='NF ','01','09')))
      cArt		:=  IIF ( TRB->IMPOSTO='RET',IIF(TRB->CONVENIO='1','002','003'),'009')
      cLetra   	:= 	LEFT(TRB->SERIE,1,1)
      cNro_comp := 	IIF(TRB->IMPOSTO='RET',StrZero(val(TRB->COMPROB),16),StrZero(val(TRB->COMPROB),12)+SPACE(4))
      cFEchacomp:= 	cFecPer
      ...
      //IF EMPTY(cNro_ins,'2','1') //Si no tiene cargado Nro. IB lo considera como No Inscripto
      ...
      cstring 	:= ctipo_op 	+ 	cFecper 	+  ;
                 cCod_nor 		+	ctipocom 	+ 	cLetra   	+ cNro_comp + ;
                 cFechacomp  	+ 	cMonto 		+ 	cTip_doc 	+ ;
                 cNro_doc 		+ 	cSit_ib 	+ ;
                 cNro_ins 		+ 	cSit_iva 	+ cMarcaOG 		+ cMarcaDrel + ;
                 cImpOG    		+  	cImpIva		+ cMon_neto 	+ cAlicuota 	+ ;
                 cRet_prac		+  	cImpIva 	+  cRet_prac	+ cArt + Chr(13)+ Chr(10)
      ...
      //datos para impresion      
      IF Len(aDet) == 0      
	      AADD(aDet,{ {"T.Op.","C" }		 ,{"Cod.Nor.","C"}	 	 ,{"Fec.Retenc.","D" },;
	                  {"Tip.Compr.","C" }	 ,{"Serie","C"}  		 ,{"Nro Comprobante","C"},;
	                  {"Fec.Comprob.","D" }	 ,{"Importe ","N"}		 ,{" ","C"}	,;
	                  {"T.Doc.","C"}   	 	 ,{"Docum./CUIT","C"}  	 ,{"Sit.IB","C"}		,;
	                  {"Nro Insc.","C"}	 	 ,{"Sit.IVA","C"}  	 	 ,{"Razon Social","C"}	,; 
	                  {" ","C"}  	 		 ,{" ","C"}	         	 ,{"Monto Neto","N"}	,;                                                  
	                  {"Alic.","N"}		 	 ,{"Retenc.","N"}	     ,{"Total Retenc.","N"}} )           
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ListFil   ºAutor  ³ms					 ºFecha ³  09/19/10   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Lista filiales de la empresa  		                      º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ AP                                                         º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
...
While !SM0->( EOF() )
	// Solo adiciona en aRecSM0Fil si la empresa es igual a la empresa actual
	If SM0->M0_CODIGO == cEmpAnt
	   aAdd(aRecSM0Fil,{ Recno(), SM0->M0_CODIGO , SM0->M0_CODFIL, ALLTRIM(SM0->M0_FILIAL)})
	EndIf
