...
±±ºPrograma  ³M020INC   ºAutor  ³Microsiga           º Data ³  12/25/11   º±±
...
User Function M020INC()
	...
	If inclui
		For nX:= 1 To SA2->(FCount())
			aAdd(aSA2,{ SA2->(FieldName(nX)), SA2->(FieldGet(nX)), Posicione("SX3",2,SA2->(FieldName(nX)),"X3_TIPO")})			
		Next nX
	Else	
		Return()	
	EndIf
	...
	While SM0->(!Eof())
		...
		EndIf
 	  	...
 	  	If oServer:Connect(cLocal,nPorta) 
		 	...
		 	oServer:CallProc("RPCSetEnv",SM0->M0_CODIGO,SM0->M0_CODFIL,,,,, {"SA2"})
		 	lDel:=(nxOpc == 2)
	 	 	LjMsgRun("Espere, actualizando Proveedor en la empresa "+SM0->M0_CODIGO+" - "+SM0->M0_CODFIL,"",{|| oServer:CallProc('U_MANSA2',aSA2,lDel)})
            cCount :=1 
...
User Function MANSA2(aSA2,lDel)
	
	Local nX			:= 1
	Local aStruct		:= SA2->(DbStruct())
	Local nPos			:= 0
	Local nPosCod		:= aScan( aSA2,{ |x| Alltrim(x[1]) == "A2_COD" } )	 
	Local nPosloja		:= aScan( aSA2,{ |x| Alltrim(x[1]) == "A2_LOJA"  } )	
	...
	DbSelectArea( "SA2" )
	SA2->(DbSetOrder(1))
	If SA2->(DbSeek(xFilial("SA2")+ aSA2[nPosCod][2] +aSA2[nPosLoja][2]))
		lLock	:= .F.		
	EndIf
	
	//Caso seja delecao
	If lDel .And. !lLock
		Conout(Time()+" - Borrando Proveedor "+Alltrim(SA2->A1_COD)+"- "+DTOS(DDATABASE))
	...
	ElseIf !lDel
		RecLock("SA2",lLock)
			...
			For nX := 1 to Len(aStruct)
		  		...
		  		nPos:= aScan( aSA2,{ |x| Alltrim(x[1]) == Alltrim(aStruct[nX][1]) } )	
				...
				If Alltrim(aSA2[nPos][3]) <> Alltrim(aStruct[nX][2]) 
					Loop
				EndIf
					
				//Os campos abaixo esta com bug na funcao fieldput
				If Alltrim(aStruct[nX][1]) $ "A1_FILIAL"
					SA2->A1_FILIAL 	:= xFilial("SA2")
					Loop
				EndIf
				
				lRetRegra  := .T.
				_xConteudo := aSA2[nX][2]
				_xNome     := aSA2[nX][1]
				xCampo	   := aScan( aSA2,{ |x| Alltrim(x[1]) == aStruct[nX,1] } )
				xContenido := aSA2[xCampo][2]
				
				//Verifica regra de excecao para o campo
		   //		xConteudo	:= u_RegraCampo( _xConteudo , SA2->(FieldGet(nX)) , _xNome , lLock , xFilial("SA2") , _cModo , @lRetRegra )
