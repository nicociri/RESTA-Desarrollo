	...
	While SM0->(!Eof())
		...
		EndIf
 	  	...
 	  	If oServer:Connect(cLocal,nPorta) 
		 	...
		 	lDel:=(nxOpc == 2)
	 	 	LjMsgRun("Espere, actualizando producto en la empresa "+SM0->M0_CODIGO+" - "+SM0->M0_CODFIL,"",{|| oServer:CallProc('U_MANSA1',aSA1,lDel)})
            cCount :=1 
...
User Function MANSA1(aSA1,lDel)
	...
	DbSelectArea( "SA1" )
	SA1->(DbSetOrder(1))
	If SA1->(DbSeek(xFilial("SA1")+ ALLTRIM(aSA1[nPosCod][2]) +ALLTRIM(aSA1[nPosLoja][2])))
		lLock	:= .F.		
	EndIf
	
	//Caso seja delecao
	If lDel .And. !lLock
		Conout(Time()+" - Borrando producto "+Alltrim(SA1->A1_COD)+"- "+DTOS(DDATABASE))
